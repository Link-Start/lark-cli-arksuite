...
Supports `--as user` or `--as bot`. The caller must have read and manage access to the target whiteboard, otherwise the subscribe OAPI returns 403 and `event consume` exits with an auth error before listening.

## `board.whiteboard.updated_v1`

### Per-whiteboard subscription

Unlike global event keys (e.g. minutes / im), this key subscribes **per whiteboard**: `event consume` calls `POST /open-apis/board/v1/whiteboards/{whiteboard_id}/subscribe` on startup with the `whiteboard_id` you pass via `-p`. **Required parameter**: `-p whiteboard_id=<whiteboard_token>`. Missing this param fails the PreConsume step with a `param whiteboard_id is required` error before any subscription happens.
...
| `.event.operator_ids[].user_id` | string (kind=user_id) | Editor's user_id (only present when caller has `contact:user.employee_id:readonly` scope) |
...
| Phase | Behavior |
|---|---|
| Startup | `event consume` calls `subscribe` OAPI; on success stderr emits `[event] running pre-consume setup...` then `[event] ready event_key=board.whiteboard.updated_v1` |
...
lark-cli event consume whiteboard.whiteboard.updated_v1 \
